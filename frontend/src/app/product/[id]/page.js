  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8">
      ...
      <nav className="text-sm text-gray-500 mb-6">
        <Link href="/" className="hover:text-vikas-blue">Home</Link> {' / '}
        <Link href="/products" className="hover:text-vikas-blue">Products</Link> {' / '}
      ...
      <div className="grid md:grid-cols-2 gap-8">
        ...
        {/* Product Info */}
        <div>
          <p className="text-sm text-vikas-blue font-medium mb-1">{product.brand}</p>
          ...
          <div className="flex items-baseline gap-3 mb-6">
            <span className="text-3xl font-bold text-vikas-dark">₹{parseFloat(product.price).toLocaleString()}</span>
          ...
          <div className="flex gap-3 mb-8">
            <button onClick={addToCart} className="flex-1 flex items-center justify-center gap-2 bg-vikas-blue text-white py-3 rounded-lg font-semibold hover:bg-blue-800 transition">
              <FiShoppingCart /> Add to Cart
            </button>
            <button onClick={buyNow} className="flex-1 bg-vikas-orange text-white py-3 rounded-lg font-semibold hover:bg-orange-600 transition">
              Buy Now
            </button>
          </div>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      {s.available > 0 && (
                        <button onClick={() => { setSelectedStore(s); setShowReservation(true); }}
                          className="text-xs bg-vikas-blue text-white px-3 py-1.5 rounded-lg hover:bg-blue-800 font-medium">
                          Reserve
                        </button>
                      )}
                    </div>
      ...
      <button onClick={() => setShowChat(!showChat)}
        className="fixed bottom-6 right-6 bg-vikas-blue text-white p-4 rounded-full shadow-lg hover:bg-blue-800 transition z-50">
